...
import TicketNFT from '../abis/ticketing.json';
import Ticketing from '../abis/ticketnft.json';
...
export const TicketNFTAddress = '0xcb13F063904feb0A967277802bAECc7548C053C3';
const TicketingAddress = '0x6E42f62471C3D731cb5152212757b395C4962988';
...
export const buyTicket = async (
  ...
  const bookingContract = await TicketingContract();
  const connectedAccount = getGlobalState('connectedAccount');

  try {
    setLoadingMsg('Purchase ticket');
    const bought = await bookingContract?.buyTicket(eventId, category, {
      from: connectedAccount,
      value: ticketPrice,
    });
    //@ts-ignore
...
const getAllEvents = async () => {
  ...
  // fetch all events
  try {
    const allEvents = await bookingContract?.getAllEvents();
...
export const getMyEvents = async () => {
  ...
  if (address) {
    try {
      ...
      const counter = await bookingContract?.myEventCount(address);

      for (let i = 0; i < counter; i++) {
        const singleEvent = await bookingContract?.myEvents(address, i + 1);
...
export const getMyTickets = async () => {
  ...
  if (address)
    try {
      ...
      const counter = await bookingContract?.orderCount(address);

      for (let i = 0; i < counter; i++) {
        const ticket = await bookingContract?.myOrders(address, i + 1);
...
export const renderQRcode = async (ticket_id, type = 'blob') => {
  ...
  const textString = 'Zora Ticket #' + ticket_id,
